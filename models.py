...
class Player(UserMixin, db.Model):
    ...
    def to_dict(self):
        # include global rank so the caller can show standings.  rank is the
        # number of players with a strictly higher rating, plus one.
        rank = (
            db.session.query(Player)
            .filter(Player.rating > self.rating)
            .count() + 1
        )
